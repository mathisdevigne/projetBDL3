...
insert into contenu (titre, description, duree, pays, anneeProd, classification, themeprincipal) 
    select d.title, 
           d.description, 
           (case d.type 
                when 'TV Show' then null 
                else to_number(substr(d.duration, 1, instr(d.duration, ' ')-1)) 
            end), 
           (case  
...
insert into contenu_type (refContenu, nomType)
    select distinct c.ref, (case 
                    ...
                    when (d.type = 'Movies' and (d.listed_in like '%Stand-Up Comedy%' or d.listed_in like '%Stand-Up Comedy & Talk Shows%')) then 'Divertissement'
                    else 'Cinéma'
                  end)
...
insert into joue (idPers, refContenu)
    ...
    where ct.refContenu = c.ref and 
          ...
          (ct.nomType = 'Cinéma' or ct.nomType = 'Series')        
;
